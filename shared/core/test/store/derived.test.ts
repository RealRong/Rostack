...
describe('createDerivedStore', () => {
    test('keeps the published snapshot reference when recompute stays equal and still refreshes dependencies', () => {
        ...
        const right = store.value({
            count: 1
        });
        const derived = store.value(() => store.read(active) === 'left'
            ? store.read(left)
            : store.read(right), {
    ...
    test('allows listener side effects that invalidate dependencies after recompute commits', () => {
        const left = store.value(0);
        const right = store.value(0);
        const total = store.value(() => store.read(left) + store.read(right));
        const values: number[] = [];
    ...
    test('still throws on direct circular derived dependencies', () => {
        ...
        const left = store.value(() => store.read(right) + 1);
        right = store.value(() => store.read(left) + 1);
        expect(() => {
            left.get();
