...
describe('node edit selection chrome', () => {
  ...
  it('keeps node drag and toolbar style writes working for a selected shape', () => {
    ...
    const beforeRect = editor.read.node.render.get('shape-1')?.rect
    ...
    expect(editor.read.node.render.get('shape-1')?.rect).toMatchObject({
      x: 128,
      y: 152
    })
    ...
    expect(editor.read.node.render.get('shape-1')?.node.style).toMatchObject({
      fill: '#22c55e',
      stroke: '#ef4444',
      strokeWidth: 3
    })
  ...
  it('keeps root drag working for a selected mindmap-owned topic', () => {
    ...
    const beforeRect = editor.read.node.render.get('root-1')?.rect
    expect(beforeRect).toBeDefined()
    ...
    expect(editor.read.node.render.get('root-1')?.rect).toMatchObject({
      x: 248,
      y: 220
    })
  })
  ...
  it('keeps mindmap topic style writes working for a selected root topic', () => {
    ...
    expect(editor.read.node.render.get('root-1')?.node.style).toMatchObject({
      frameKind: 'underline',
      stroke: '#ef4444',
      strokeWidth: 3,
      fill: '#22c55e'
