...
describe('mindmap enter animation', () => {
  it('publishes intermediate topic positions while the enter preview is active', async () => {
    ...
    try {
      ...
      const baseRect = editor.scene.query.scene.node(inserted.data.nodeId)?.geometry.rect
      ...
      const endRect = editor.scene.query.scene.node(inserted.data.nodeId)?.geometry.rect
