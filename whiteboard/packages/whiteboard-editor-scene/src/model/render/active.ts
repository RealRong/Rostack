...
import { patchFamilyReset, patchFamilyTouched } from './family'
...
const buildActiveView = (input: {
  ...
  return {
    ...
    box: edge.box,
    state: {
      hovered: input.context.current.runtime.interaction.hover.kind === 'edge'
        && input.context.current.runtime.interaction.hover.edgeId === input.edgeId,
      selected: input.context.current.runtime.interaction.selection.edgeIds.includes(input.edgeId),
