...
describe('node edit selection chrome', () => {
  ...
  it('keeps toolbar and overlay visible while editing a selected mindmap-owned root topic', async () => {
    ...
    editor.actions.session.edit.startNode('root-1', 'text')
  ...
  it('keeps node drag and toolbar style writes working for a selected shape', async () => {
    ...
    expect(editor.scene.nodes.get('shape-1')?.geometry.rect).toMatchObject({
      x: 128,
      y: 152
    })
    ...
    editor.actions.document.node.style.fill(scope!.node!.nodeIds, '#22c55e')
    editor.actions.document.node.style.stroke(scope!.node!.nodeIds, '#ef4444')
    editor.actions.document.node.style.strokeWidth(scope!.node!.nodeIds, 3)
    ...
    expect(editor.scene.nodes.get('shape-1')?.base.node.style).toMatchObject({
      fill: '#22c55e',
      stroke: '#ef4444',
      strokeWidth: 3
    })
  ...
  it('keeps root drag working for a selected mindmap-owned topic', async () => {
    ...
    expect(editor.scene.nodes.get('root-1')?.geometry.rect).toMatchObject({
      x: 248,
      y: 212
    })
  })
  ...
  it('keeps mindmap topic style writes working for a selected root topic', async () => {
    ...
    const result = editor.actions.document.mindmap.style.topic({
    ...
    expect(editor.scene.nodes.get('root-1')?.base.node.style).toMatchObject({
      frameKind: 'underline',
      stroke: '#ef4444',
      strokeWidth: 3,
      fill: '#22c55e'
