...
import { rectIn } from '@shared/dom';
import { hitTestBoardLayout } from '@dataview/react/views/kanban/drag';
import { useDrag } from '@dataview/react/views/kanban/drag';
import type { KanbanBoard, KanbanViewRuntime } from '@dataview/react/views/kanban/types';
...
import { useKanbanLayout } from '@dataview/react/views/kanban/runtime/layout';
import { useKanbanVisibility } from '@dataview/react/views/kanban/runtime/visibility';
const EMPTY_SECTIONS = [] as readonly Section[];
const sameBoard = (left: KanbanBoard, right: KanbanBoard) => left.viewId === right.viewId
...
export const useKanbanRuntime = (input: {
    ...
    const configStore = useMemo(() => store.value({
    ...
    const board = useMemo(() => store.value<KanbanBoard>(() => {
