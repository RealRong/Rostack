...
import { hoveredCellOf, hoveredRowIdOf, sameHoverTarget, type TableHoverTarget } from '@dataview/react/views/table/model/hover';
...
export const createTableHover = (): TableHoverRuntime => {
    const state: store.ValueStore<HoverState> = store.value<HoverState>({
        target: null,
        pointer: null
    }, {
        isEqual: (left, right) => (sameHoverTarget(left.target, right.target)
            && equal.sameOptionalPoint(left.pointer, right.pointer))
    });
    const row = store.keyed<ItemId, boolean>({
        emptyValue: false,
        isEqual: Object.is
    });
    const cellState = store.keyed<CellId, boolean>({
        emptyValue: false,
        isEqual: Object.is
    });
    const cell = store.keyed<CellRef, boolean>(current => store.read(cellState, cellId(current)), {
        keyOf: cellId,
        isEqual: Object.is
    });
    const syncMembership = (previous: TableHoverTarget | null, next: TableHoverTarget | null) => {
    ...
    const set = (target: TableHoverTarget | null, point: Point | null = store.peek(state).pointer) => {
        const current = store.peek(state);
        const previous = current.target;
        if (sameHoverTarget(previous, target)
