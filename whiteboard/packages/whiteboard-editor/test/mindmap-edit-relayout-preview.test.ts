...
describe('mindmap edit relayout preview', () => {
  ...
  it('relayouts sibling positions when topic live height grows during edit', () => {
    ...
    const beforeFirst = editor.scene.query.scene.node(first.data.nodeId)?.geometry.rect
    const beforeSecond = editor.scene.query.scene.node(second.data.nodeId)?.geometry.rect
    ...
    const liveFirst = editor.scene.query.scene.node(first.data.nodeId)?.geometry.rect
    const liveSecond = editor.scene.query.scene.node(second.data.nodeId)?.geometry.rect
  ...
  it('notifies subscribed sibling render during live topic relayout', () => {
    ...
    const unsubscribe = editor.scene.stores.render.node.byId.subscribe(second.data.nodeId, () => {
      const rect = editor.scene.query.scene.node(second.data.nodeId)?.geometry.rect
      if (!rect) {
        return
      }
    ...
    expect(notifications.at(-1)?.y).toBe(
      editor.scene.query.scene.node(second.data.nodeId)?.geometry.rect.y
    )
  })
})
