...
export const WHITEBOARD_OPERATION_DEFINITIONS: WhiteboardOperationDefinitionTable = {
  'document.replace': {
    family: 'document',
    sync: 'checkpoint',
...
export const whiteboardMutationOperations: MutationOperationsSpec<
  Document,
  Operation,
  HistoryFootprint[number],
  WhiteboardOperationReduceExtra,
  ...
  WhiteboardReduceIssueCode
> = {
  table: WHITEBOARD_OPERATION_DEFINITIONS,
...
>({
  spec: {
    serializeKey: whiteboardMutationOperations.serializeKey,
    createContext: whiteboardMutationOperations.createContext,
    handle: (ctx, operation) => {
      const definition = WHITEBOARD_OPERATION_DEFINITIONS[operation.type]
      definition.footprint?.(ctx, operation as never)
      definition.apply(ctx, operation as never)
    },
    settle: whiteboardMutationOperations.settle,
    done: whiteboardMutationOperations.done
  }
})
