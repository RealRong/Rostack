...
import type { Box, Point } from '@shared/dom';
import { rectFromPoints } from '@shared/dom';
import { equal, store as coreStore } from '@shared/core';
import type { ItemSelectionController, ItemSelectionSnapshot, OrderedSelectionDomain, SelectionScope, SelectionSummary } from '@dataview/runtime/selection';
import { selectionSnapshot } from '@dataview/runtime/selection';
import { createNullableControllerStore } from '@dataview/runtime/session/controller';
...
const sameHitIds = (left: readonly ItemId[], right: readonly ItemId[]) => left.length === right.length
    && left.every((id, index) => id === right[index]);
const sameSession = (left: MarqueeSessionState | null, right: MarqueeSessionState | null) => {
    if (left === right) {
        return true;
    }
    if (!left || !right) {
...
const resolveBaseSelected = (input: {
    ...
    id: ItemId;
}) => selectionSnapshot.contains(input.domain, input.session.baseSelection, input.id);
...
export const createMarqueeController = (input: {
    ...
    const { store: sessionStore, get, clear, openStore } = createNullableControllerStore<MarqueeSessionState>({
        isEqual: sameSession
    });
    const previewMembershipStore = coreStore.keyed<ItemId, boolean | null>({
        emptyValue: null,
        isEqual: Object.is
    });
    const previewSummaryStore = coreStore.keyed<SelectionScope<ItemId>, SelectionSummary | null>(scope => {
    ...
    const resolveNextSelection = (session: MarqueeSessionState) => {
        const domain = input.resolveDomain();
        return domain
            ? selectionSnapshot.applyIds(domain, session.baseSelection, session.mode, session.hitIds, input.selection.state.getSnapshot().domainRevision)
            : session.baseSelection;
    };
    const clearPreviewMembership = () => {
        previewMembershipStore.clear();
    };
    const seedPreviewMembership = (session: MarqueeSessionState) => {
        ...
        previewMembershipStore.patch({
            set: Array.from(selectionSnapshot.iterate(domain, session.baseSelection), id => [id, false] as const)
        });
    };
    const syncPreviewMembership = (inputValue: {
        ...
        const setEntries: Array<readonly [
            ItemId,
            boolean | null
        ]> = [];
