...
import type { TableBody, TableColumn } from '@dataview/runtime';
...
const createTableDisplayedFields = (columns: readonly TableColumn[]): TableDisplayedFields => {
...
const sameDisplayedFields = (left: TableDisplayedFields | undefined, right: TableDisplayedFields | undefined) => left === right || (!!left
    && !!right
    && left.all.length === right.all.length
    && left.all.every((field, index) => field === right.all[index]));
export const createTableDisplayedFieldsStore = (bodyStore: store.ReadStore<TableBody | null>): store.ReadStore<TableDisplayedFields | undefined> => {
    let previousColumns: readonly TableColumn[] | undefined;
    let previous: TableDisplayedFields | undefined;
    return store.value<TableDisplayedFields | undefined>(() => {
