...
export const createEditorChromeUi = (input: {
  ...
  const marquee = store.createDerivedStore({
    get: () => input.scene.overlay.marquee(),
    isEqual: (left: EditorMarqueePreview | undefined, right: EditorMarqueePreview | undefined) => (
      left === right
      || (
  ...
  const draw = store.createDerivedStore({
    get: () => input.scene.overlay.draw()
  })

  const snapGuides = store.createDerivedStore({
    get: () => input.scene.overlay.guides()
  })

  const edgeGuide = store.createDerivedStore({
    get: () => input.scene.overlay.edgeGuide() ?? EMPTY_EDGE_GUIDE,
