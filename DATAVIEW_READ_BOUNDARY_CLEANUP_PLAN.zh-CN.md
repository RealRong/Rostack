...
## 4.2 `MutationEngine` core 不需要这么大

这里必须再收紧一个关键判断：

> 上一节讨论的是系统最终需要的“读边界能力”，不等于这些能力都应该直接挂在 `MutationEngine` core 上。

如果只追求长期最优，`shared/mutation` 里的 `MutationEngine` 应该更像一个**写入内核**，而不是“写入 + 读取 + 生命周期 + history”的万能对象。

### `MutationEngine` core 最终只需要的能力

```ts
interface MutationEngineKernel<Intent, Op, Write> {
  execute(
    intent: Intent | readonly Intent[],
    options?: MutationOptions
  ): MutationResult<unknown, Write>

  apply(
    ops: readonly Op[],
    options?: MutationOptions
  ): MutationResult<void, Write>

  readonly writes: {
    subscribe(listener: (write: Write) => void): () => void
  }
}
```
...
- `execute`
  - 本地领域命令入口。
  - 如果需要 batch intent，长期建议直接让它支持 `Intent | Intent[]`，而不是额外保留一个 `executeMany`。
- `apply`
  - remote replay、history undo/redo、canonical op 注入入口。
  - 这是 mutation kernel 必须保留的第二条主入口。
- `writes.subscribe`
  - 唯一值得保留在 kernel 上的观测面。
  - collab、history、外层 runtime 都可以从 write stream 派生。

### 哪些不该继续留在 core

- `executeMany`
  - 不值得与 `execute` 并存。
  - 要么合并进 `execute(intent[])`，要么退化成外层 wrapper。
- `doc`
  - 只是 `current` 的弱化版，长期没有独立存在价值。
- `current`
  - 名字像普通 getter，实际是 snapshot 语义，是最容易被内部误用的 API。
- `subscribe`
  - 现在它分发的是 full snapshot 风格数据，不该继续存在于 kernel。
- `history`
  - 这是本地附属系统，不是 mutation kernel 的本体职责。
- `load`
  - 这是 bootstrap/import/reset 生命周期能力，不是 steady-state mutation 能力。

## 4.3 读边界应该作为 adapter，而不是 core surface

系统仍然需要 raw read / snapshot / export 这些能力，但它们不该直接长在 `MutationEngine` core 上。

更合理的分层是：

```ts
interface MutationReadAdapter<Doc, Publish, InternalState> {
...
也就是说，系统确实需要这些 API，但它们属于 **read adapter**，不属于 **mutation kernel**。

## 4.4 生命周期与 history 也应该外移

除了 read adapter，还应再拆出两类外围能力：
...
这些都属于“把一个外部文档装进内核”的边界行为，不该和 steady-state mutation API 混在一起。
...
- 本地 undo / redo 控制器
- checkpoint 策略
- 基于 `writes` 的 capture / clear / reconcile

`history` 可以继续复用 mutation kernel 的 write stream，但不该以公开属性的形式直接挂在 kernel 上。
...
## 6.4 `MutationEngine` API 收敛结论

为了避免后续再次把 adapter 能力塞回 kernel，这里把最终裁剪结论明确写死：
...
- `executeMany`
  - 合并进 `execute(intent[])` 或下沉成 wrapper
- `doc`
  - 删除
- `current`
  - 删除
...
- `history`
  - 移到 history adapter
- `load`
  - 移到 lifecycle adapter
- `snapshot/export/readState/readDoc/readPublish/subscribeRaw/subscribeSnapshot`
  - 移到 read adapter

最终应形成下面这种明确结构：

```text
MutationEngineKernel
  只负责 execute / apply / writes

MutationReadAdapter
  负责 raw read / snapshot / export / raw subscribe / snapshot subscribe

MutationLifecycleAdapter
  负责 load / replace / import / reset

MutationHistoryAdapter
  负责 undo / redo / checkpoint / capture policy
```
