...
const equalCapabilities = (left: Capabilities, right: Capabilities) => (left.canHover === right.canHover
...
export const createCapabilities = (options: {
...
}): store.ReadStore<Capabilities> => store.value<Capabilities>(() => {
    ...
    const canHover = !locked && (interaction.mode === 'idle'
        || interaction.mode === 'keyboard');
    const showFillHandle = !locked && (interaction.mode === 'idle'
        || interaction.mode === 'keyboard'
        || interaction.gesture === 'cell-select');
    return {
        canHover,
        canRowDrag: !locked && Boolean(body
