...
import { observeElementSize, pageScrollNode, scrollByClamped, scrollMetrics, viewportRect, type ScrollNode } from '@shared/dom';
...
import { createTableMeasurementPlan, EMPTY_TABLE_MEASUREMENT_PLAN, sameTableMeasurementPlan, type TableMeasurementPlan } from '@dataview/react/views/table/virtual/measurementPlan';
import { createTableLayoutState, sameTableLayoutState, type TableLayoutState } from '@dataview/react/views/table/virtual/layoutState';
import type { TableBlock } from '@dataview/react/views/table/virtual/types';
import type { TableBody } from '@dataview/runtime';
import type { TableBlockId } from '@dataview/react/views/table/virtual/blockId';
const BOOTSTRAP_VIEWPORT_HEIGHT = () => (typeof window !== 'undefined'
...
export type TableVerticalDirection = typeof SCROLL_UP | typeof SCROLL_IDLE | typeof SCROLL_DOWN;
...
const sameLayoutSnapshot = (left: TableVirtualLayoutSnapshot, right: TableVirtualLayoutSnapshot) => left.totalHeight === right.totalHeight
    && left.revision === right.revision
    && left.rowCount === right.rowCount;
const sameViewportSnapshot = (left: TableVirtualViewportSnapshot, right: TableVirtualViewportSnapshot) => left.ready === right.ready
...
const sameInteractionSnapshot = (left: TableVirtualInteractionSnapshot, right: TableVirtualInteractionSnapshot) => left.marqueeActive === right.marqueeActive
    && left.overscanBefore === right.overscanBefore
    && left.overscanAfter === right.overscanAfter;
const sameWindowBlock = (left: TableBlock, right: TableBlock) => {
...
const sameWindowSnapshot = (left: TableVirtualWindowSnapshot, right: TableVirtualWindowSnapshot) => left.totalHeight === right.totalHeight
...
export const resolveTableWindowSnapshot = (input: {
    ...
    const start = Math.max(0, input.viewport.viewportTopInCanvas - input.interaction.overscanBefore);
    const end = Math.max(start, input.viewport.viewportBottomInCanvas + input.interaction.overscanAfter);
...
const resolveMarqueeActive = (input: {
    ...
    active: boolean;
}) => Boolean(input.layoutState
    && input.active);
const createTableLayoutStateStore = (input: {
...
}) => store.value<TableLayoutState | null>(() => {
...
export const createTableVirtualRuntime = (options: {
    ...
    const measurementPlanStore = store.value<TableMeasurementPlan>(() => {
    ...
    const layoutStore = store.value<TableVirtualLayoutSnapshot>(EMPTY_LAYOUT_SNAPSHOT, {
        isEqual: sameLayoutSnapshot
    });
    const viewportStore = store.value<TableVirtualViewportSnapshot>(createBootstrapViewportSnapshot(), {
        isEqual: sameViewportSnapshot
    });
    const interaction = store.value<TableVirtualInteractionSnapshot>(() => {
    ...
    const windowStore = store.value<TableVirtualWindowSnapshot>(EMPTY_WINDOW_SNAPSHOT, {
    ...
    const measureViewport = (flags: number) => {
        ...
        const verticalDirection = deltaTop > 0
            ...
            : deltaTop < 0
                ? SCROLL_UP
                : SCROLL_IDLE;
        if (flags & MEASURE_LAYOUT
            || canvasTopInScrollContent === null) {
            const pageViewport = viewportRect(scrollNode);
            const canvasRect = canvas.getBoundingClientRect();
            const viewportTopInCanvas = Math.max(0, metrics.top - (canvasRect.top
                - pageViewport.top
                + metrics.top));
            canvasTopInScrollContent = (canvasRect.top
                - pageViewport.top
                + metrics.top);
    ...
    const syncMeasurements = (input: {
        ...
        const hasPatchChanges = Boolean(input.changedHeightById?.size
        ...
        const didApply = hasPatchChanges
            ...
            : layoutModel.replaceMeasuredHeights(measurementBuckets.get(currentBucketKey) ?? new Map<string, number>());
        if (!didApply) {
            return;
        }
        layoutRevision += 1;
    ...
    const attach = () => {
        ...
        const boundViewportElement = (typeof Window !== 'undefined' && boundScrollNode instanceof Window)
            ? null
        ...
        cleanupListeners = store.join(unsubscribes);
        measureViewport(MEASURE_LAYOUT
    ...
    return {
        ...
        hitRows: input => (layoutModel?.materializeWindow({
            start: input.top,
            end: input.bottom
        }).items.flatMap(block => (block.kind === 'row'
            ? [block.rowId]
            : [])) ?? []),
        measurement: {
