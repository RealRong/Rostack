import type { CardLayout, CardSize, CustomField, CustomFieldId, DataDoc, DataRecord, Field, FieldId, KanbanCardsPerColumn, RecordId, ValueRef, View, ViewId } from '@dataview/core/types';
import { TITLE_FIELD_ID } from '@dataview/core/types';
import type { EngineCommit } from '@dataview/engine/contracts/write';
import type { ActiveViewGallery, ActiveViewKanban, ActiveViewQuery, ActiveViewTable, ViewState } from '@dataview/engine/contracts/view';
import type { EngineSource, EntitySource, ListedEntitySource, SectionSource } from '@dataview/engine/contracts/source';
import type { FieldList, ItemId, ItemList, ItemPlacement, Section, SectionId, SectionList } from '@dataview/engine/contracts/shared';
import { collection, equal, store } from '@shared/core';
import type { createDataviewProjection } from '@dataview/engine/projection';
...
const readDocumentField = (document: DataDoc, fieldId: FieldId): Field | undefined => fieldId === TITLE_FIELD_ID
    ? TITLE_FIELD
    : document.fields.byId[fieldId];
const readDocumentValue = (document: DataDoc, ref: ValueRef): unknown => {
...
const createProjectionSelectorSubscription = (projection: ProjectionRuntime): SubscribeSource<ViewState | undefined> => (listener) => projection.subscribe(() => {
...
export const createEngineSource = (input: {
    ...
    const activeItemRecord = store.keyed<ItemId, RecordId | undefined>((itemId) => store.read(activeItems.byId, itemId)?.recordId);
    const activeItemSection = store.keyed<ItemId, SectionId | undefined>((itemId) => store.read(activeItems.byId, itemId)?.sectionId);
