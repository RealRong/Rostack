...
从架构师角度看，dataview 和 whiteboard 当前这两套底层设计，对各自场景都是合理的，也都可以做到高性能。
...
这非常适合 dataview，因为 dataview 的主要成本不是逐像素交互，而是：

- 变更后重新计算“当前数据视图”

换句话说，dataview 的性能重点是：
...
换句话说，whiteboard 的性能重点是：

- 高效地维护一个实时可交互的空间场景

## 第三判断：两套系统在底层本来就不该强行统一
...
- dataview 的单 store 快照模型
- whiteboard 的 impact 驱动 projection 模型
...
说 whiteboard 不该做单 store，并不等于 whiteboard 现在完全不用收敛。

whiteboard 当前仍然有一个明确可优化点：
...
但这不是在否定 whiteboard 的增量 read 架构，恰恰相反，这种收敛应该建立在保留它现有优势的前提上。

## 对 dataview 的额外说明

说 dataview 适合单 store，也不等于它只能走这一条路。
...
如果未来 dataview 真出现类似 whiteboard 的高频空间投影场景，再引入更局部的增量缓存层也不是不行。

但那将是新问题驱动的新设计，不应该提前为了“和 whiteboard 统一”就改变现有正确方向。

## 最终结论

从架构适配性看：

- dataview 当前的单状态快照路线是合理且高性能的
- whiteboard 当前的 impact 驱动增量投影路线也是合理且高性能的
...
对 dataview，单 store 是顺势而为。

对 whiteboard，增量 projection 是顺势而为。

它们应该共享方法论，不应该共享同一种底层物理形态。
