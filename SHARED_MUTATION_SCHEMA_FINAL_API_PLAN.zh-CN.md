...
### 1. 继续收缩 compile surface

compile 已经切到 `read / write / query / change / issue / services` 主体，但还需要继续收口。

需要继续做：

1. 检查 `shared/mutation/src/compile/types.ts`，把仍然偏重的公开类型继续内收
2. 确认 `ctx.change.current()` / `ctx.change.changes(...)` 是否还能进一步简化成更直接的最终 API
3. 删除 compile 层里任何仅为旧调用方式残留的包装命名或类型桥
4. 保证 handler 侧只面向 schema facade，不暴露 internal write / internal delta 细节

完成标准：

- compile public types 只剩最小必需集合
- handler 上下文没有多余桥接概念
...
### 4. mutation 完成后的 collab conflict scope 设计
