...
export const createActiveRuntimeTrace = (input: {
  previous: ViewState | undefined
  next: ViewState | undefined
  runtimeTrace: TraceRun<ActivePhaseName, ActivePhaseMetrics>
}): {
  view: ViewTrace
  ...
  const stagesByName = new Map(
    input.runtimeTrace.phases.map(phase => [phase.name, phase] as const)
  )
  ...
  return {
    view: {
      ...
      timings: {
        totalMs: input.runtimeTrace.totalMs
      },
      stages
    },
