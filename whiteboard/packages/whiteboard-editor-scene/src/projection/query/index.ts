...
export const createProjectionRead = (runtime: {
  ...
  const nodes: SceneNodes = {
    ...
    entries: () => runtime.state().graph.nodes.entries(),
    ...
    owner: (nodeId) => runtime.state().indexes.ownerByNode.get(nodeId)
  }
  ...
  const selection: SceneSelection = createSelectionRead({
    state: runtime.state,
    spatial,
  ...
  const groups: SceneGroups = {
    ...
    entries: () => runtime.state().graph.owners.groups.entries(),
