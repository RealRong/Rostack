...
import type { KanbanModel, KanbanBoard, KanbanCard, KanbanSection } from '@dataview/runtime/model/kanban/types';
...
const sameBoard = (left: KanbanBoard | null, right: KanbanBoard | null) => left === right || (!!left
    && !!right
...
const sameSection = (left: KanbanSection | undefined, right: KanbanSection | undefined) => left === right || (!!left
    && !!right
    && left.id === right.id
...
const sameCard = (left: KanbanCard | undefined, right: KanbanCard | undefined) => left === right || (!!left
    && !!right
    && left.viewId === right.viewId
...
export const createKanbanModel = (input: {
    ...
    const sections = store.value(() => (store.read(input.source.active.viewType) === 'kanban'
    ...
    const board = store.value<KanbanBoard | null>(() => {
    ...
    const section = store.keyed<string, KanbanSection | undefined>(key => {
        if (store.read(input.source.active.viewType) !== 'kanban') {
            return undefined;
        }
    ...
    const card = store.keyed<ItemId, KanbanCard | undefined>(itemId => {
        if (store.read(input.source.active.viewType) !== 'kanban') {
            return undefined;
        }
        const viewId = store.read(input.source.active.viewId);
