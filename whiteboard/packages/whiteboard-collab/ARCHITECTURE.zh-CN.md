...
- 监听 `engine.commits`
- 把本地 `apply commit.forward` 与 `apply commit.footprint` 发布成 shared change
...
- 本地语义写入的共享载体是 `apply commit.forward`
- 远端消费也只回放 `ops`
...
1. 监听 `engine.commits`
2. 忽略 `origin === 'remote'`
3. 如果收到 `replace commit`，写 checkpoint 并清空 tail log
4. 如果收到 `apply commit` 且 `forward` 含 checkpoint op，写 checkpoint 并清空 tail log
5. 否则把 `apply commit.forward` 过滤成 `SharedOperation[]`
6. 用 `apply commit.footprint` 组装 `SharedChange`
7. append 到 Yjs `changes`
...
因为 `apply commit.forward` 本身就是一次完整的语义写入结果。
...
- 一条 `SharedChange` 对应一次本地 `apply commit`
