...
import { cellId, sameOptionalCell, type CellId } from '@dataview/runtime';
...
export const createTableFillRuntime = (input: {
    ...
    const handle = store.value<CellRef | undefined>(() => {
    ...
    const state = store.keyed<CellId, boolean>({
        emptyValue: false,
        isEqual: Object.is
    });
    const cell = store.keyed<CellRef, boolean>(current => store.read(state, cellId(current)), {
        keyOf: cellId,
        isEqual: Object.is
    });
