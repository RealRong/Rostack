...
1. `ActivePatch` 应该从 runtime 主 contract 中明确删除。
2. `ViewRuntimeResult` 不再对外返回 `active patch`。
3. `currentView.patch` 应该从 runtime state 删除。
4. `source` 直接基于 `previousSnapshot / nextSnapshot` 做本地同步。
5. 如果还需要 diff，它只能是 `source` 内部私有实现，不再跨层暴露。

最终链路应该改成：

1. `snapshot` 继续作为真源
2. `publish` 只负责产出 `ViewState`
3. `source.sync(previousSnapshot, nextSnapshot)` 在 sink-local 做同步

也就是说，不再保留：

1. `publish(snapshot) -> buildActivePatch(...)`
2. `source.apply(activePatch)`

而是改成：

1. `publish(snapshot)`
2. `source.sync(previousSnapshot, nextSnapshot)`

需要额外说明的是：

1. 这里明确删除的是 `ActivePatch` 这层中间语言。
2. 不是要求 `source` 完全不要 diff。
3. 也不是要求每次都把整份 `snapshot` 全量灌进所有 store。
4. 真正的目标是把 diff 收口成 sink-local 实现细节，而不是整条 runtime 的公共语言。

`DocumentPatch` 可以暂时保留。

原因是：

1. `DocumentPatch` 直接来自 commit impact，对 document source 的投影比较直接。
2. `ActivePatch` 则是从 `snapshot` 二次翻译出来的，重复性更强，删除收益更大。
...
3. `keys(id)` 背后可以有 cache，但 cache 不需要成为公开主结构。
...
1. 接收 `previousSnapshot` 和 `nextSnapshot`
2. 在 source 侧本地完成同步
3. 如果需要 diff，就在 source 内部生成并立即消费
4. 如果需要 item identity cache，也在这里维护

也就是说：

1. `snapshot` 是语义真源
2. `source` 是发布和订阅真源
3. `ActivePatch` 不再存在于 runtime 主链
4. diff 只作为 source 内部私有实现存在
...
如果最终一定还需要 diff，也应当是 sink-local diff，而不是 runtime-wide patch 语言。

这样浪费的只是 source 侧的一点局部计算，但换来的是：

1. snapshot contract 更干净
2. publish 更单纯
3. `ActivePatch` 不再反向影响上游语义结构
4. 50k filter 变化时那一段纯 patch 翻译成本可以直接被消掉
...
4. `ViewRuntimeResult.patch`
5. `currentView.patch`
6. runtime-wide 的 `ActivePatch` 中间语言

同时改成：

1. `source.sync(previousSnapshot, nextSnapshot)`
2. source 内部私有 diff 或局部同步
