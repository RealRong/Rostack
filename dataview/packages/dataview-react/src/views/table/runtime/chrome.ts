...
const sameRowChrome = (left: TableRowChrome, right: TableRowChrome) => left.selected === right.selected
    && left.exposed === right.exposed
    && left.canDrag === right.canDrag;
const sameCellChrome = (left: TableCellChrome, right: TableCellChrome) => left.selected === right.selected
...
export const createTableChromeRuntime = (input: {
    ...
    row: store.keyed<ItemId, TableRowChrome>(rowId => ({
    ...
    cell: store.keyed<CellRef, TableCellChrome>(cell => {
