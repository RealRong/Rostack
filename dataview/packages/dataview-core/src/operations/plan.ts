...
export interface RecordCreateFilterRule {
  fieldId: FieldId
  field?: Field
  rule: FilterRule
  effective: boolean
...
export interface BuildRecordCreateIntentsInput {
  recordId: RecordId
  values?: Partial<Record<FieldId, unknown>>
...
const toRecordCreateInput = (input: {
  ...
  for (const [fieldId, value] of Object.entries(input.draft.values) as [FieldId, unknown][]) {
    ...
    values[fieldId as CustomFieldId] = value
  }
...
export const recordCreate = buildRecordCreateIntents
