...
const createEmptyChange = <TKey,>(): IdChangeSet<TKey> => changeSet.create<TKey>()
...
export const publishEntityFamily = <TKey extends string, TValue>(input: {
  ...
  input.change.removed.forEach((id) => {
    ...
    changeSet.markRemoved(nextChange, id)
  })
  ...
  input.change.added.forEach((id) => {
    ...
    if (previousValue === undefined) {
      changeSet.markAdded(nextChange, id)
      return
    }

    changeSet.markUpdated(nextChange, id)
  })
  ...
  const changed = !idsEqual || changeSet.hasAny(nextChange)
  if (!changed) {
    return {
