...
import { createListenerSet, createNullableControllerStore } from '@dataview/runtime/session/controller';
export interface InlineSessionTarget {
    viewId: ViewId;
    itemId: ItemId;
}
export type InlineSessionExitReason = 'submit' | 'escape' | 'outside' | 'selection' | 'view-change' | 'programmatic';
...
const inlineSessionKey = (target: InlineSessionTarget) => `${target.viewId}${INLINE_SESSION_SEPARATOR}${target.itemId}`;
const sameTarget = (left: InlineSessionTarget | null, right: InlineSessionTarget | null) => {
    if (left === right) {
        return true;
    }
    if (!left || !right) {
...
export const resolveInlineSessionExitEffect = (reason: InlineSessionExitReason): InlineSessionExitEffect => {
    switch (reason) {
        case 'submit':
        case 'outside':
        case 'selection':
...
export const createInlineSessionApi = (initial: InlineSessionTarget | null = null): InlineSessionApi => {
    const { store, get } = createNullableControllerStore<InlineSessionTarget>({
        initial,
        isEqual: sameTarget
    });
    const editing = coreStore.keyed<string, boolean>(key => {
