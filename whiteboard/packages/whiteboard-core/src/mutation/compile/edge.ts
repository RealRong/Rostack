...
const emitEdgeRouteDiffOps = (
  ...
  if (currentMiddle.length === 0) {
    ...
    nextMiddle.forEach((point) => {
      ...
      ctx.writer.edge.route(edgeId).insert(
        routePoint,
  ...
  if (nextMiddle.length === 0) {
    currentMiddle.forEach((point) => {
      ctx.writer.edge.route(edgeId).delete(point.id)
    })
    return
  }
  ...
  if (currentMiddle.length === nextMiddle.length) {
    currentMiddle.forEach((point, index) => {
      ...
      if (Object.keys(fields).length) {
        ctx.writer.edge.route(edgeId).patch(point.id, fields)
      }
    })
    return
  }
...
const emitEdgeUpdateInputOps = (
  ...
  ctx.writer.edge.patch(edge.id, createEdgePatch({
    ...(fields ? { fields } : {}),
    ...(record ? { record } : {})
  }))
}
...
const compileEdgeRouteDelete = (
  ...
  ctx.writer.edge.route(edge.id).delete(pointId)
}
...
export const edgeIntentHandlers = {
  ...
  'edge.update': (ctx) => {
    ...
    ctx.intent.updates.forEach((entry) => {
      const edge = ctx.reader.edge.get(entry.id)
      if (!edge) {
        return
      }
  ...
  'edge.move': (ctx) => {
    ...
    intent.ids.forEach((edgeId) => {
      const edge = ctx.reader.edge.get(edgeId)
  ...
  'edge.reconnect.commit': (ctx) => {
    ...
    const edge = ctx.reader.edge.get(intent.edgeId)
    if (!edge) {
      return
    }
  ...
  'edge.label.insert': (ctx) => {
    ...
    ctx.writer.edge.labels(edge.id).insert(
      label,
      ctx.intent.to
  ...
  'edge.label.update': (ctx) => {
    ...
    ctx.writer.edge.labels(edge.id).patch(label.id, createEdgeLabelPatch({
      ...(fields ? { fields } : {}),
      ...(record ? { record } : {})
    }))
  },
  'edge.label.move': (ctx) => {
    ctx.writer.edge.labels(ctx.intent.edgeId).move(
      ctx.intent.labelId,
      toMutationOrderedAnchor(ctx.intent.to)
    )
  },
  'edge.label.delete': (ctx) => {
    ctx.writer.edge.labels(ctx.intent.edgeId).delete(ctx.intent.labelId)
  },
  'edge.route.insert': (ctx) => {
    ...
    ctx.writer.edge.route(edge.id).insert({
      id: pointId,
  ...
  'edge.route.update': (ctx) => {
    ...
    ctx.writer.edge.route(edge.id).patch(point.id, fields)
  },
  'edge.route.set': (ctx) => {
    const edge = ctx.expect.edge(ctx.intent.edgeId)
    if (!edge) {
  ...
  'edge.route.move': (ctx) => {
    ctx.writer.edge.route(ctx.intent.edgeId).move(
      ctx.intent.pointId,
      toMutationOrderedAnchor(ctx.intent.to)
    )
  },
