...
```ts
interface MutationCompileContext<Doc, Intent, Reader, Tag extends string = string> {
  intent: Intent
  document: Doc
  reader: Reader
...
type MutationIntentCompiler<Doc, Intent, Reader, Tag extends string = string> = (
  context: MutationCompileContext<Doc, Intent, Reader, Tag>
) => void
```

这里 `program` 是唯一写入口，compile 直接往里面写 step。
...
compile 直接产出 shared program step：
...
compile 直接写 shared program step：
...
## Phase 2：Dataview core 直接写 program
...
## Phase 3：Whiteboard core 直接写 program
...
## Phase 4：引入 editor state engine
